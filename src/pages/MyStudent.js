...
const MyStudent = () => {
  ...
  useEffect(() => {
    const paymentInfo = async() => {
      const response = await AxiosApi.studentGet(userName);
  ...
  return(
    ...
    <p>수강생 관리</p>
    <hr />
    <li>수강생 내역은 최근 1년을 기준으로 조회됩니다.</li>
    <li>수강 관리 상세 문의는 고객센터를 통해 빠른 처리를 도와드리겠습니다.</li>
    <li className="last-list">수강생 정보 유출에 각별한 주의를 부탁드립니다.</li>
    ...
    <table>
      ...
      {currentItem.length === 0 ? (
        <tr>
         <td colspan="5" style={{textAlign: "center"}}>수강 신청 내역이 없습니다</td>
        </tr>
